...
class Module1Lesson10Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val scrollView = binding.findViewById<NestedScrollView>(R.id.fragment_module1_lesson10)
        val position = args.position+1

        val recyclerView = binding.findViewById<RecyclerView>(R.id.lesson10_list_view)
        recyclerView.layoutManager = LinearLayoutManager(context)
        recyclerView.adapter = Adapters(parseLesson(resources,R.xml.basic_module1_lesson10), resources, requireContext())
